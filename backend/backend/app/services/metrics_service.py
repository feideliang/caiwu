...
class MetricsService:
    ...
    async def get_core_metrics(
        ...
        # Customer data (always needed for concentration metrics)
        ...
        if (need_customer_bd or need_summary) and not _is_customer_dim:
            cust_q = select(AggDimensionSummary).where(
                AggDimensionSummary.period.in_(list(detail_periods)),
                AggDimensionSummary.bgbu == bgbu_filter,
                AggDimensionSummary.dim_type == "customer",
            )
            cust_rows = (await db.execute(cust_q)).scalars().all()
            for row in cust_rows:
                period_customer_bucket[row.period][row.dim_value]["revenue"] += float(row.revenue or 0)
                period_customer_bucket[row.period][row.dim_value]["cost"] += float(row.cost or 0)
                period_customer_bucket[row.period][row.dim_value]["gross_profit"] += float(row.gross_profit or 0)
                period_customer_rev[row.period][row.dim_value] += float(row.revenue or 0)
        ...
        # Product data (always needed for concentration metrics)
        ...
        if (need_breakdowns or need_summary) and not _is_product_dim:
            prod_q = select(AggDimensionSummary).where(
                AggDimensionSummary.period.in_(list(detail_periods)),
                AggDimensionSummary.bgbu == bgbu_filter,
                AggDimensionSummary.dim_type == "sales_product",
            )
            prod_rows = (await db.execute(prod_q)).scalars().all()
            for row in prod_rows:
                period_product_bucket[row.period][row.dim_value]["revenue"] += float(row.revenue or 0)
                period_product_bucket[row.period][row.dim_value]["cost"] += float(row.cost or 0)
                period_product_bucket[row.period][row.dim_value]["gross_profit"] += float(row.gross_profit or 0)
                period_product_gp[row.period][row.dim_value] += float(row.gross_profit or 0)
                period_product_rev[row.period][row.dim_value] += float(row.revenue or 0)
        ...
        if need_breakdowns or need_summary:
            ord_q = select(AggOrderSummary).where(
                AggOrderSummary.period.in_(list(order_detail_periods)),
                AggOrderSummary.bgbu == bgbu_filter,
            )
            ord_rows = (await db.execute(ord_q)).scalars().all()
            for row in ord_rows:
                period_order[row.period][row.order_id]["revenue"] += float(row.revenue or 0)
                period_order[row.period][row.order_id]["cost"] += float(row.cost or 0)
                period_order[row.period][row.order_id]["gross_profit"] += float(row.gross_profit or 0)
        ...
        # Populate period_orders_dim from order_summary for dept/product dimensions
        if need_breakdowns:
            if _is_dept_dim and bgbu_filter == "ALL":
                # Per-department order counts from agg_order_summary grouped by dim_dept
                ord_dim_q = select(
                    AggOrderSummary.period, AggOrderSummary.dim_dept, AggOrderSummary.order_id
                ).where(
                    AggOrderSummary.period.in_(list(current_members)),
                    AggOrderSummary.bgbu == "ALL",
                )
                for p, dept, oid in (await db.execute(ord_dim_q)).all():
                    if dept and oid:
                        period_orders_dim[str(dept)].add(oid)
            elif _is_product_dim:
                ord_dim_q = select(
                    AggOrderSummary.period, AggOrderSummary.dim_product, AggOrderSummary.order_id
                ).where(
                    AggOrderSummary.period.in_(list(current_members)),
                    AggOrderSummary.bgbu == bgbu_filter,
                )
                for p, prod, oid in (await db.execute(ord_dim_q)).all():
                    if prod and oid:
                        period_orders_dim[str(prod)].add(oid)
        ...
        if cust_top3 is not None and mom_period and prev_rev:
            ...
            if mom_cust:
                ...
                mom_top3 = mom_top3_sum / prev_rev * 100
        ...
        # Product concentration top3 (revenue, denominator = total product revenue only)
        ...
        if prod_rev and total_prod_rev:
            ...
            prod_top3 = top3_sum / total_prod_rev * 100

        # Product concentration top10 (revenue)
        prod_top10 = None
        if prod_rev and total_prod_rev:
            top10_sum = sum(sorted(prod_rev.values(), reverse=True)[:10])
            prod_top10 = top10_sum / total_prod_rev * 100
        ...
        # Achievement rate (target_rev already computed via SQL above)
        ...
        # Direct sign customer metrics (already computed via SQL above)
        if direct_sign_rev:
