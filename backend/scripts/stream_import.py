"""
流式导入 Excel -> income_margin_detail
read_only=True 边读边写，内存占用低
"""
import openpyxl
import asyncpg
import asyncio
...
async def main():
    ...
    print(f"Opening {FILE} (read_only)...")
    ...
    col_index = {}  # en_name -> excel_col_index
    for i, cn in enumerate(header):
        if cn in COL_MAP:
            col_index[COL_MAP[cn]] = i

    missing = [c for c in INSERT_COLS[2:] if c not in col_index]
    if missing:
