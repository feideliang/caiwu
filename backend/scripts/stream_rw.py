"""
流式读写：openpyxl read_only 边读边写，不全部加载到内存
"""
import openpyxl
import asyncpg
import asyncio
...
async def main():
    ...
    print(f"Opening {FILE} (read_only, max_col={MAX_COL})...")
    wb = openpyxl.load_workbook(FILE, read_only=True)
    ws = wb.active

    # 读 header，手动指定 max_col
    rows_iter = ws.iter_rows(min_row=1, max_row=1, max_col=MAX_COL, values_only=True)
    header = next(rows_iter)
    print(f"  Header: {len(header)} cols")
    print(f"  First 5: {[str(h) for h in header[:5]]}")

    # 建立列索引
    col_index = {}
    for i, cn in enumerate(header):
        if cn and cn in COL_MAP:
            col_index[COL_MAP[cn]] = i

    rcd_idx = col_index.get("revenue_confirm_date")
    yr_idx = col_index.get("revenue_year")
    mo_idx = col_index.get("revenue_month")
    co_idx = col_index.get("company")
    ...
    for row in ws.iter_rows(min_row=2, max_col=MAX_COL, values_only=True):
        ...
        # period
        rcd = row[rcd_idx] if rcd_idx is not None and rcd_idx < len(row) else None
        if rcd and not (isinstance(rcd, float) and rcd != rcd):
            period = str(rcd)[:7]
        else:
            yr = row[yr_idx] if yr_idx is not None and yr_idx < len(row) else None
            mo = row[mo_idx] if mo_idx is not None and mo_idx < len(row) else None
            if yr and mo:
                period = f"{int(yr)}-{int(mo):02d}"
            else:
                period = "unknown"

        entity = row[co_idx] if co_idx is not None and co_idx < len(row) else None
        entity = str(entity) if entity else None

        vals = [period, entity]
        for col in INSERT_COLS[2:]:
            idx = col_index.get(col)
            raw = row[idx] if idx is not None and idx < len(row) else None
            vals.append(clean(raw, col))
        ...
        if len(batch) >= BATCH:
            ...
            print(f"  {total:,} rows [{time.time()-t1:.1f}s]")
            batch.clear()
