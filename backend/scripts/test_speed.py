"""测试写入 3000 行的速度"""
import pandas as pd, asyncpg, asyncio, time, math
...
COL_MAP = {
    ...
    "产品事业部名称": "product_bu_name", "产品所属事业部": "product_bgbu",
    "产品所属组织": "product_org", "产品系列": "series", "产品线": "product_bgbu",
...
INSERT_COLS = [
    ...
    "product_bgbu", "product_org", "series", "product_bgbu",
...
async def main():
    t0 = time.time()
    print("Reading 3000 rows...")
    df = pd.read_excel(FILE, nrows=3000, engine="openpyxl")
    ...
    t1 = time.time()
    await conn.copy_records_to_table("income_margin_test", columns=INSERT_COLS, records=records)
    tw = time.time() - t1
    cnt = await conn.fetchval("SELECT count(*) FROM income_margin_test")
    print(f"Write {cnt} rows: {tw:.2f}s")
